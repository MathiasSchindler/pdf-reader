...
JPEG and decoded image XObjects honor same-size, 8-bit DeviceGray `/SMask` images with Flate or JPEG compression, including `/Decode` polarity. An image whose soft mask cannot be decoded is reported and skipped rather than drawn opaque over existing page content.
